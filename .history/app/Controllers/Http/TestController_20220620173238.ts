    ...
    if (
      (/* Checking if the loan is authorized for payout and termination, the request type is payout
      loan, the approval status is approved, and the status is payout. */
      loan.length > 0 &&
      ...
      (/* Checking if the loan is authorized for payout, not authorized for termination, the request
      type is payout loan, the approval status is approved, and the status is payout. */
      loan.length > 0 &&
        loan[0].isPayoutAuthorized === true &&
        ...
        loan[0].approvalStatus === "approved" &&
        loan[0].status === "payout") ||
      (/* Checking if the loan is terminated. */
      loan.length > 0 &&
      ...
      if (
        ...
        if (rolloverType === "100") {
          ...
          /* Checking if the loan is already paid out. If it is not, it will create a new loan record.
          If it is, it will update the existing loan record. */
          await testingFunction(payoutRequestIsExisting);
                ...
                case "101":
                  ...
                  if (rate === undefined) {
                    ...
                    timelineObject = {
                      ...
                      createdAt: DateTime.now(),
                      meta: `amount invested: ${loan[0].amount},amount paid: ${
...
async function testingFunction(payoutRequestIsExisting: any) {
