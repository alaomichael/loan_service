...
export default class LoansController {
  ...
  public async store({ request, response }: HttpContextContract) {
    ...
    const loanSchema = schema.create({
      ...
      phone: schema.string(),
      email: schema.string(),
      savingsAccountNumber: schema.string(),
    ...
    console.log("Payload line 1217  :", payload);
    // check BVN status
...
if(payload.firstName + payload.laNstname){
  return response.json({status:"FAILED", message: "user details does not match, please check your details and try again."})
    ...
    if (approvalIsAutomated === false || approvalIsAutomated.toString() === "0") {
      ...
      console.log("Payload line 1258  :", payload);
      ...
      console.log("Timeline object line 1296:", timelineObject);
            let newTimeline = await Timeline.create(timelineObject);

      console.log(" NEW TIMELINE line 1299 :", newTimeline);

      await loan.save();

      // Send Approval Request to Admin
      let approval = await approvalRequest(walletId, loanId, requestType);
      console.log(" Approval request return line 1305 : ", approval);
      if (approval === undefined) {
        return response.status(400).json({
          status: "FAILED",
          message:
      ...
      amountRecommended = payload.amountRequested;
      // call Okra
